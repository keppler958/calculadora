 ...
 buttons.forEach (b => {
  ...
  if (e.key === 'Enter' || e.key === '=') {
     ...
     if (!isNaN(res)) {
        ...
        screen.value = Number.isInteger(rea) ? res : res.toFixed(2);
  ...
  if (e.keytoLowerCase() === 'd')
  {
    const newMode = document.
    documentElement.classiList.contains
    ('dark') ? 'light' : 'dark';
    setTheme(newMode);
    e.preventDefault();
  }  
  if (e.key.toLowerCase() === 'h'){
    ...
    ('tranlate-y-full') || historyPanel.classList.contains('sm:tranlate-x-full')
